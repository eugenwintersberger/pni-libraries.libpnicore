...
namespace core{
    ...
    */
    ...
    This function parses CLI options an arguments from a vector. Unlike the
    original pointer approach the vector must not contain the program name
    (usually passed as the first argument to a program). A utilty function can
    be used to convert the defautl CLI argument pointer to a vector
    \code
