...
public:
	...
	virtual void setShape(const ArrayShape &s)=0;
	virtual const ArrayShape &getShape() const =0;

	virtual void setBuffer(const BufferObject &b) = 0;
	virtual const BufferObject &getBuffer()=0;
...
template<typename T> class Array:public DataObject{
    protected:
...
template<typename T> Array<T>::Array(const unsigned int &r,const unsigned int s[]):DataObject()
{
...
template<typename T> Array<T>::Array(const Array<T> &a):DataObject(a)
{
    //set shape object
...
template<typename T> Array<T>::Array(const ArrayShape &s):DataObject()
{
...
template<typename T> Array<T>::Array(const boost::shared_ptr<ArrayShape> &s):DataObject()
{
...
template<typename T> Array<T>::Array(const ArrayShape &s,const Buffer<T> &b):DataObject()
{
