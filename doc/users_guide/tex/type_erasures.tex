...
The \cpp{value} type encapsulates data of an arbitrary type and has full 
...
\cpp{std::reference\_wrapper}. Unfortunately, this template includes the 
full type information -- which is what we want to get rid of when using 
a type erasure. 
\libpnicore\ for this purpose provides the \cpp{value\_ref} erasure. It stores 
a reference to an existing data item and hides all the type information. 
Though \cpp{value\_ref} behaves quite similar to \cpp{value} there are some 
...
Like \cpp{value}, \cpp{value\_ref} is default constructible
\begin{cppcode}
value_ref vref;
\end{cppcode}
allowing it to be used in STL containers. However, unlike \cpp{value} the default 
constructed reference points to nowhere. Every access to any of
\cpp{value\_ref}s methods will throw \cpp{memory\_not\_allocated\_error} 
for a default constructed instance of \cpp{value\_ref}. The preferred way of 
how to initialize \cpp{value\_ref} is by passing an instance of 
\cpp{std::reference\_wrapper} to it 
\begin{cppcode}
float64 data;
value_ref data_ref(std::ref(data));
\end{cppcode}
In addition \cpp{value\_ref} is copy constructible. 

\subsection{Assignment}

The most difficult operation with \cpp{value\_ref} is assignment. It really
...
\cpp{std::reference\_wrapper} where the reference is copied. 

Another possibility is to assign the value of a primitive type to an 
instance of \cpp{value\_ref}. In this case two things are taking place
\begin{enumerate}
\item the value is converted to the type of the data item the instance 
of \cpp{value\_ref} references
...
temp_ref = uint16(12);
\end{cppcode}
In this example the value $12$ of type \cpp{uint16} is first converted to 
a \cpp{float32} value. This new float value is then assigned to the variable 
\cpp{temperature}. As always with type conversions exceptions will be 
thrown if the conversion fails.

One can also change the variable an instance of \cpp{value\_ref} references 
with
\begin{cppcode}
...
\end{cppcode}
Finally a value from a \cpp{value} instance can be assigned with
\begin{cppcode}
value v = int32(100);
value_ref ref = ....;
...
ref = v;
\end{cppcode}
in which case type conversion from the internal type of \cpp{v} to the internal 
type of \cpp{ref} occurs. Exceptions are thrown if the type conversion fails.

\subsection{Retrieving data}

Data retrieval for \cpp{value\_ref} works exactly the same way as for
\cpp{value}. The type provides a template method \cpp{as} which can be 
...
Finally, as \cpp{value}, \cpp{value\_ref} provides a \cpp{type\_id} member 
