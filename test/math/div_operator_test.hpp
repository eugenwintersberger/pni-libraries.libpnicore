 ...
 */
template<typename ATYPE> class div_operator_test: public CppUnit::TestFixture 
{
        CPPUNIT_TEST_SUITE(div_operator_test<ATYPE>);
...
//-----------------------------------------------------------------------------
template<typename ATYPE> void div_operator_test<ATYPE>::tearDown() { }

//-----------------------------------------------------------------------------
template<typename ATYPE> void div_operator_test<ATYPE>::test_construction()
{
    std::cout<<BOOST_CURRENT_FUNCTION<<std::endl;

    div_op<array_type,array_type> op(a1,a2);
    CPPUNIT_ASSERT(a1.size() == op.size());
   
    div_op<array_type,scalar_type> op2(a1,s1);
    CPPUNIT_ASSERT(op2.size() == a1.size());

    div_op<scalar_type,array_type> op3(s1,a1);
    CPPUNIT_ASSERT(op3.size() == a1.size());
}

//-----------------------------------------------------------------------------
template<typename ATYPE> void div_operator_test<ATYPE>::test_access()
{
    std::cout<<BOOST_CURRENT_FUNCTION<<std::endl;

    div_op<array_type,array_type> op1(a1,a2);
...
//-----------------------------------------------------------------------------
template<typename ATYPE> void div_operator_test<ATYPE>::test_iterator()
{
    std::cout<<BOOST_CURRENT_FUNCTION<<std::endl;
    ...
    div_op<scalar_type,array_type> op2(s1,a1);
    auto siter = s1.begin();
    iter1 = a1.begin();
#ifdef NOFOREACH
    ...
    for(auto v: op2) 
#endif 
        compare(v,(*siter++)/(*iter1++));
}

//-----------------------------------------------------------------------------
template<typename ATYPE> void div_operator_test<ATYPE>::test_operator()
{
    std::cout<<BOOST_CURRENT_FUNCTION<<std::endl;
    ...
    for(auto v: r) 
#endif
        compare(v,value_type(100)/(*iter1++));

    //put it all together
    r = value_type(100)/a1/a2;
    iter1 = a1.begin();
    iter2 = a2.begin();
#ifdef NOFOREACH
    BOOST_FOREACH(auto v,r)
    ...
    for(auto v: r) 
#endif 
        compare(v,value_type(100)/(*iter1++)/(*iter2++));

}

//-----------------------------------------------------------------------------
template<typename ATYPE> void div_operator_test<ATYPE>::test_operator_on_view()
{
    std::cout<<BOOST_CURRENT_FUNCTION<<std::endl;
